...
class ChromadbRepository:
    ...
    def add(self, diary_id: int, sentence: str) -> None:
        ...
        if self.collection is None:
            raise ValueError(
                "コレクションがロードされていません。load_collection() を先に呼び出してください。"
            )
    ...
    def find_by_sentence(self, sentence: str, top_k: int = 2) -> dict:
        ...
        if self.collection is None:
            raise ValueError(
                "DBがロードされていません。load_collection関数を呼び出してください。"
            )

        return self.collection.query(query_texts=[sentence], n_results=top_k)
