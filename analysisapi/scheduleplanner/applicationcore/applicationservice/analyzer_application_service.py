...
class AnalyzerAplicationService:
  """
  日記データをAIで解析するクラス(作成中)。
  """
  def __init__(self, json_data: dict = None, user_id: int=None, is_vectorize: bool=None):
    """
    コンストラクタ
  ...
  def set_openai_api(self, api_key: str):
    # OpenAI APIの初期化
    self.client = OpenAI()
    self.client.api_key = os.getenv("OPENAI_API_KEY")
  ...
  def _add_text_to_db(self):
    ...
    """
    # 文章をベクトル化してベクトルDBに格納
    self.vectorizer = ChromadbRepository(user_id=self.json_data["diaries"][0]["userId"], persist=False)
